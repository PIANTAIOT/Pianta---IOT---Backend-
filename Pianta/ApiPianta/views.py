...
User = get_user_model()
# Se obtiene la clase de modelo de usuario actualmente en uso mediante la función `get_user_model()` y se asigna a la variable `User`.

# Clase `MyUserDetailsView` que hereda de `UserDetailsView`.
class MyUserDetailsView(UserDetailsView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Se extrae el valor del argumento 'partial' de los parámetros kwargs, si está presente, de lo contrario se establece en False.

        instance = self.get_object()
        # Se obtiene la instancia del objeto del usuario actual basado en la solicitud.

        serializer = self.get_serializer(instance, data=request.data, partial=partial)
        # Se crea una instancia del serializador definido en `serializer_class`.
        # Se pasan los datos de la solicitud (request.data) y el valor de 'partial' al serializador.

        serializer.is_valid(raise_exception=True)
        # Se verifica si los datos del serializador son válidos.
        # Si los datos no son válidos, se genera una excepción de validación (raise_exception=True).

        serializer.save()
        # Se guarda la instancia del objeto actualizada utilizando el serializador.

        return Response(serializer.data)
        # Se devuelve una respuesta con los datos del serializador después de guardar la instancia actualizada.
...
class CustomPasswordResetConfirmView(PasswordResetConfirmView):
    ...
    # Nombre del archivo de plantilla que se utilizará para renderizar la vista de confirmación de restablecimiento de contraseña.
    form_class = CustomPasswordResetForm
    # Clase de formulario que se utilizará para procesar los datos de la solicitud de confirmación de restablecimiento de contraseña.
    queryset = User.objects.all()
    # Consulta para obtener todos los objetos de usuario.
    success_url = reverse_lazy('password_reset_complete')
    # URL de redirección después de que se haya completado con éxito el restablecimiento de contraseña.
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Se crea una instancia del formulario definido en `form_class`.
        # Se pasan los valores del uid y uidb64 al formulario como argumentos.

        form.fields['uid'].initial = kwargs['uidb64']
        # Se establece el valor inicial del campo 'uid' del formulario.

        context['form'] = form
        # Se agrega el formulario al contexto.

        return render(request, self.template_name, context)
        # Se renderiza la plantilla utilizando el contexto y se devuelve la respuesta renderizada.

    def get_object(self, queryset=None):
        uidb64 = self.kwargs.get('uidb64') # Obtiene el valor 'uidb64' de los argumentos de palabras clave (kwargs)
        token = self.kwargs.get('token') # Obtiene el valor 'token' de los argumentos de palabras clave (kwargs)
        if queryset is None:
            queryset = self.get_queryset()# Obtiene el conjunto de consultas si no se proporciona ninguno
            uidb64 = self.kwargs.get('uidb64')# Actualiza el valor 'uidb64' de los argumentos de palabras clave (kwargs)
        try:
            uid = urlsafe_base64_decode(uidb64).decode()# Decodifica 'uidb64' utilizando urlsafe_base64_decode y lo convierte a cadena
            user = User.objects.get(pk=uid)# Obtiene el objeto User con la clave primaria 'uid'
        except (TypeError, ValueError, OverflowError, User.DoesNotExist):
            user = None # Si se produce alguna excepción o el usuario no existe, se establece user como None
        
        return user # Devuelve el objeto User encontrado o None si no se encuentra
    
    def post(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data) # Crea una instancia del serializador con los datos de la solicitud
        serializer.is_valid(raise_exception=True) # Valida el serializador y genera una excepción si no es válido
        serializer.save() # Guarda los datos validados del serializador

        # Define un contexto con un mensaje para la plantilla
        context = {
            'message': _('Your password has been reset successfully.'),
        }
        # Renderiza la plantilla 'password_reset_confirm.html' con el contexto definido
...
class CustomLoginView(LoginView):
    def get_response_data(self, user):
        response_data = super().get_response_data(user)  # Obtiene los datos de respuesta predeterminados del método padre

        response_data['last_login'] = user.last_login  # Agrega la información de 'last_login' al diccionario de datos de respuesta

        # Si el usuario acaba de iniciar sesión, actualiza last_login a la hora actual
        user.last_login = timezone.now()  # Establece last_login del usuario como la hora actual utilizando el objeto 'timezone'
        user.save()  # Guarda los cambios realizados en el objeto 'user'

        return response_data  # Devuelve el diccionario de datos de respuesta modificado
...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            # Obtener el email del serializador validado
            email = serializer.validated_data['email']
            # Verificar si el usuario ya existe en la base de datos
            user_exists = User.objects.filter(email=email).exists()

            if user_exists:
                # Si el usuario ya existe, retornar un mensaje de error con estado 409 (Conflicto)
            ...
            if username:
                ...
                if username_exists:
                    # Si el nombre de usuario ya está en uso, retornar un mensaje de error con estado 409 (Conflicto)
            ...
            # Preparar los datos de respuesta con el usuario y el token
            data = {
                'user': NewLoginSerializer(user).data,
                'token': str(token)
            }

            # Retornar los datos de respuesta con estado 201 (Creado)
            return Response(data, status=status.HTTP_201_CREATED)
        else:
            # Si el serializador no es válido, retornar los errores con estado 400 (Solicitud incorrecta)
...
def send_registration_email(request):
    if request.method == 'POST':
        ...
        if email:
            ...
            # Construir el mensaje de correo electrónico con el token generado
            message = f'Hola, para continuar con el registro, por favor ingrese siguiente código: {token}, este código tendrá 10 minutos de caducidad, así que úsalo en este momento.'
            ...
            # Retornar una respuesta HTTP con el mensaje indicando el resultado del envío del correo
...
def validate_token(request):
    if request.method == 'POST':
        ...
        try:
            # Obtener el objeto del token de la base de datos
            token_obj = TokensEmail.objects.get(token=user_token, is_valid=True)
        except Token.DoesNotExist:
            # Si no se encuentra el token en la base de datos, retornar una respuesta JSON indicando que el token es inválido
            return JsonResponse({'success': False, 'message': 'Token inválido.'})

        if token_obj.is_expired():
            # Si el token ha expirado, marcarlo como no válido en la base de datos y generar un nuevo token
            token_obj.is_valid = False
            token_obj.save()
            TokensEmail.create_token()
            return JsonResponse({'success': False, 'message': 'El token ha expirado.'})
        else:
            # Si el token es válido y no ha expirado, actualizar su fecha de caducidad en la base de datos
            token_obj.expires_at = timezone.now() + timedelta(minutes=10)
            token_obj.save()
            
            # Retornar una respuesta JSON indicando que el token se ha verificado con éxito
            return JsonResponse({'success': True, 'message': 'Token verificado con éxito.'})
    else:
        # Si el método de la solicitud no es POST, retornar una respuesta JSON indicando que solo se admiten solicitudes POST
